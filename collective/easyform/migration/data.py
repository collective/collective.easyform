...
def should_migrate_row(data_adapter, easyform, row, idx):
...
def is_valid_value(key, value, easyform):
...
def migrate_row(data_adapter, easyform, row):
    cols = data_adapter.getColumnNames()
    data = {}
    for key, value in zip(cols, row):
        is_valid_value(key, value, easyform)
        data[key] = value
    return data
    ...
    for data_adapter in ploneformgen.objectValues("FormSaveDataAdapter"):
        ...
        if ISaveData.providedBy(action):
            ...
            for idx, row in enumerate(data_adapter.getSavedFormInput()):
                if not should_migrate_row(data_adapter, easyform, row, idx):
                    ...
                    continue
                try:
                    data = migrate_row(data_adapter, easyform, row)
